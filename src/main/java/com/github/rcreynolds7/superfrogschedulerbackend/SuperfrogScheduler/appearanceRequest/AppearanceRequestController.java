...
public class AppearanceRequestController {
    ...
    /*
    //This is UC 8. It is commented out because it relies on using UC7(Ability to view an appearance request)

    @PutMapping("/{requestId}")
    public Result updateAppearanceRequestByAdmin(@PathVariable Integer requestId, @RequestBody AppearanceRequest appearanceRequestUpdate) {
    //Simulate a call to view the appearance request before editing (fictional UC-7)
    AppearanceRequest appearanceRequest = appearanceRequestService.viewAppearanceRequest(requestId);

    //Proceed with updating the request with admin privileges
    Optional.ofNullable(appearanceRequestUpdate.getFirstName())
            .filter(name -> !name.isEmpty())
            .ifPresent(appearanceRequest::setFirstName);
